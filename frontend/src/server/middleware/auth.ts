...
function getSecret(secret: string): Uint8Array { return new TextEncoder().encode(secret) }

export async function generateToken(payload: { userId: string; username: string }, secret: string): Promise<string> {
  return new SignJWT({ ...payload }).setProtectedHeader({ alg: 'HS256' }).setIssuedAt().setExpirationTime('7d').sign(getSecret(secret))
}
...
export const authMiddleware = createMiddleware<{
  ...
  try {
    const { payload } = await jwtVerify(auth.slice(7), getSecret(c.env.JWT_SECRET), { algorithms: ['HS256'] })
